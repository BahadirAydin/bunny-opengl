...
void initVBO()
{
    ...
    gVertexDataSizeInBytes = 4 * 3 * sizeof(GLfloat);  // 4 vertices for a rectangle
    gNormalDataSizeInBytes = 4 * 3 * sizeof(GLfloat);  // 4 normals for a rectangle
    int indexDataSizeInBytes = 2 * 3 * sizeof(GLuint); // 2 triangles for a rectangle
...
void display()
{
	...
	matT = glm::translate(glm::mat4(1.0), glm::vec3(5.f, 0.f, -5.f));
	matS = glm::scale(glm::mat4(1.0), glm::vec3(0.25, 0.25, 0.25));
	matR = glm::rotate<float>(glm::mat4(1.0), (-180. / 180.) * M_PI, glm::vec3(0.0, 1.0, 0.0));
	matRz = glm::rotate(glm::mat4(1.0), angleRad, glm::vec3(0.0, 0.0, 1.0));
    modelingMatrix = matT  * matS * matRz * matR;
    activeProgramIndex = 1;
	glUseProgram(gProgram[activeProgramIndex]);
	glUniformMatrix4fv(projectionMatrixLoc[activeProgramIndex], 1, GL_FALSE, glm::value_ptr(projectionMatrix));
	glUniformMatrix4fv(viewingMatrixLoc[activeProgramIndex], 1, GL_FALSE, glm::value_ptr(viewingMatrix));
